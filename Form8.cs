    ...
    public partial class Form8 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(con))
                {
                    ...
                    string insertQuery = @"INSERT INTO Appointmenttb ( PatientId, Doctorname, AppointmentDate, Appointmenttime,TestSuggest) 
                                  VALUES 
                                  ( @PatientId, @Doctorname, @AppointmentDate, @AppointmentTime,@TestSuggest)";
                    ...
                    using (SqlCommand cmd = new SqlCommand(insertQuery, connection))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@AppointmentTime", textBox5.Text);
                        cmd.Parameters.AddWithValue("TestSuggest", textBox4.Text);
                        ...
                        catch (SqlException sqlEx)
                        {
                            ...
                            if (sqlEx.Number == 547)
                            {
                                MessageBox.Show("Error: The specified DoctorId does not exist in the doctortb1 table.");
                            }
                            else
                            {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            using (SqlConnection sqlcon = new SqlConnection(con))
            {
                ...
                SqlDataAdapter sqlda = new SqlDataAdapter("select * from Appointmenttb", sqlcon);
        ...
        private void button3_Click(object sender, EventArgs e)
        {
                ...
                if (dataGridView1.SelectedRows.Count > 0)
                {
                    ...
                    // Extract AppointmentId from the selected row
                    string appointmentIdToDelete = selectedRow.Cells["AppointmentId"].Value.ToString();

                    // Execute the DELETE query
                    string deleteQuery = "DELETE FROM Appointmenttb WHERE AppointmentId = @AppointmentId";
